...
- strict linting, formatting, type checking, unit tests, build checks, dependency audit, and CI smoke test
...
Replace `JWT_ACCESS_SECRET` with a random value of at least 32 characters. Production startup should never rely on a hard-coded fallback secret.
...
GitHub Actions also performs a production dependency audit and starts the compiled API against a real PostgreSQL service to verify the readiness endpoint.
