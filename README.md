...
Current branch adds the accounting layer without exposing a public money-mutation endpoint:
...
- deferred PostgreSQL constraints that independently reject unsealed, unbalanced, or cross-currency ledger records at commit time
- concurrency coverage proving two competing debits cannot overspend one wallet
...
A valid transaction must have at least two non-zero postings, use one currency, and sum exactly to zero. Once sealed, the transaction and its postings are immutable. Customer wallet accounts may never become negative.
